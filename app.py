...
with col1:
    ...
    if len(st.session_state.posiciones_reloj) > 0:
        if st.button("🔄 HACER GIRAR EL RELOJ COGNUSS 2"):
            ...
            # Simulación visual del giro de las manecillas
            placeholder_giro = st.empty()
            for i in range(10):
                giro_falso = random.randint(1, 14)
                placeholder_giro.markdown(f"<h2 style='text-align:center; color:#7B1FA2;'>⏱️ GIRANDO AL REVOLVER: {giro_falso}</h2>", unsafe_allow_html=True)
                time.sleep(0.1)
            placeholder_giro.empty()
    ...
    st.write("")
    html_esfera = "<div style='text-align: center;'>"
    for pos in range(1, 15):
        if pos == st.session_state.cedula_activa:
            html_esfera += f"<div class='circle-slot slot-activa'>{pos:02d}</div>"
        elif pos not in st.session_state.posiciones_reloj:
            html_esfera += f"<div class='circle-slot slot-leida'>{pos:02d}</div>"
        else:
            html_esfera += f"<div class='circle-slot slot-vacia'>{pos:02d}</div>"
    html_esfera += "</div>"
...
with col2:
    ...
    if st.session_state.cedula_activa and not st.session_state.animando:
        item = CONOCIMIENTO_CEDULARIO[st.session_state.cedula_activa]
        
        st.markdown(f"<div class='box-minutero'><b style='color: #1A73E8;'>⏱️ MINUTERO (CONTEXTO):</b><br>Pregunta N° {st.session_state.cedula_activa} del Cedulario</div>", unsafe_allow_html=True)
